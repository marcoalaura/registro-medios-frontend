<template>
  <section>
    <v-card-text v-show="estadoMedio === 'PENDIENTE'">
          ...
          <v-card>
            <v-card-text>
              <v-alert type="info" :value="true">
                Por favor ingrese los detalles del paquete que ofrece.
              </v-alert>
            </v-card-text>
            <v-card-text>
              <v-layout wrap align-center>
                ...
                <v-flex xs2>
                  ...
                  <template v-if="flagHorasCrear">
                    <v-layout row wrap>
                      <v-flex xs9 sm9>
                        <selector-horas model="horaIni" empieza="00:00"></selector-horas>
                      </v-flex>
                      <v-flex xs9 sm9>
                ...
                <v-flex xs2>
                  <span>Costo del Paquete <b class="error--text">*</b></span>
                  <v-flex xs9>
                    <v-text-field
                      name="costo"
                      id="costo"
      ...
      <v-container grid-list-md v-if="tarifarioDetalle.length === 0 && nombreTipoMedio !== 'Televisión' && nombreTipoMedio !== 'Productor Independiente TV'">
        <v-card>
          <v-card-text>
            ...
            <v-layout wrap>
              ...
              <v-flex xs4>
                <template v-if="flagHoras !== false && estadoMedio === 'PENDIENTE'">
                  <v-layout row wrap>
                    <v-flex xs9 sm9>
                      <selector-horas model="horaIniEditar" :hora="horaInicio" :minuto="minutoInicio"></selector-horas>
                    </v-flex>
                    <v-flex xs9 sm9>
                      <selector-horas :inicio="false" model="horaFinEditar" :hora="horaFin" :minuto="minutoFin"></selector-horas>
                    </v-flex>
                  </v-layout>
                </template>
  ...
  methods: {
    guardarPrograma (form) {
      if (this.$refs.form.validate()) {
        this.$service.post(`medios/${this.$storage.get('medio').id}/tipos_medio/${this.idMedioTipoMedioTvRadio}/tarifarios/${this.idTarifarioTvRadio}/detalles`,
          {
        ...
        .then((response) => {
          if (response && response.detalle) {
            this.actualizaMedio();
            this.$message.success('El paquete se ha guardado correctamente.');
          ...
          this.$message.error('No se pudo guardar el paquete en el tarifario ' + error);
        });
      }
    },
    ...
    editarDetalle (formEditar) {
      if (this.$refs.formEditar.validate()) {
        ...
        .then((response) => {
          if (response && response.detalle) {
            this.actualizaMedio();
            this.$message.success('El paquete se ha actualizado correctamente.');
            this.recuperaDetalle();
            this.modalEditar = false;
          }
        }).catch((error) => {
          this.$message.error('No se pudo editar el paquete en el tarifario ' + error);
        });
      }
    },
    eliminarDetalle (dia, minutos) {
      let item = this.recuperaDatos(dia, minutos);
      this.$confirm('¿Está seguro de eliminar el paquete? El conjunto de horarios del programa se eliminarán.', () => {
        ...
        .then(response => {
          if (response.detalle) {
            this.$message.success('Paquete eliminado correctamente');
            this.actualizaMedio();
            this.recuperaDetalle();
          }
        }).catch((error) => {
          this.$message.error('No se pudo eliminar el paquete del tarifario ' + error);
        });
      });
    },
    muestraDetalle (dia, minutos) {
      ...
      .then(response => {
        ...
        this.$message.error('No se puede mostrar el paquete seleccionado ' + error);
      });
    },
