    ...
    <template v-if="estaRegistrado">
      <v-container fluid px-0>
        <v-layout row wrap>
          ...
          <v-flex xs6> 
            <v-btn color="info" @click="estaRegistrado=false" v-if="tieneMediosSeleccionados === false"><v-icon dark>autorenew</v-icon>Cambiar cobertura</v-btn>
          </v-flex>
...
<script>
import Campania from './mixins/campania';
...
export default {
  ...
  created () {
    ...
    .then(response => {
      if (response && response.campana && response.campana.data) {
        this.campanaEncontrada = response.campana.data;
        if (this.campanaEncontrada.coberturas.length > 0) {
          this.estaRegistrado = true;
          this.cobertura = this.campanaEncontrada.tipo_cobertura;
          this.campanaEncontrada.coberturas.forEach((item, index) => {
  ...
  mounted () {
    ...
    // Verificar si ya tiene una selección de medios en la base para no mostrar el botón de cambio
  },
  methods: {
    guardarCobertura () {
      if (this.idCampana !== null) {
        const urlCoberturas = `campanas/${this.idCampana}/coberturas`;
        let dataCoberturas = {};
  ...
  watch: {
    ...
    'estaRegistrado': function () {
      EventBus.$emit('estaRegistrado', this.estaRegistrado);
    }
  }
};
</script>
